...
local OnEvent = function(self, event)
	...
	elseif event == "ENCOUNTER_END" and select(2, IsInInstance()) == "raid" and (T.Wrath or T.Cata or T.Mists or T.Mainline) then
		for _, v in pairs(bars) do
			if T.Mainline or not v.duration or ((T.Wrath or T.Cata or T.Mists) and v.duration >= 120 and v.duration < 600) then
				v.endTime = 0
			end
		end
	end
