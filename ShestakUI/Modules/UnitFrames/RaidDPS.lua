...
local function Shared(self, unit)
	...
	self.Health.PostUpdateColor = function(health, unit)
		if not UnitIsConnected(unit) or (UnitIsDeadOrGhost(unit) and not UnitIsFeignDeath(unit)) then
			health:SetValue(0)
		end
	end
	...
	if suffix == "pet" or (suffix == "target" and unit ~= "tank") then
		...
		if (T.Wrath or T.Cata or T.Mists or T.Mainline) and unit == "party" and C.raidframe.icons_role ~= true then
	...
	-- LFD role icons
	if (T.Wrath or T.Cata or T.Mists or T.Mainline) and C.raidframe.icons_role == true and not (suffix == "pet" or suffix == "target") then
